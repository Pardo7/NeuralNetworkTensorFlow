...
# Implementing Min-Max scaling for grayscale image data
def normalize_grayscale(image_data):
    """
    Normalizing image data with Min-Max scaling to a range of [0.1, 0.9]
...
if not is_labels_encod:
    # Turning labels into numbers and applying One-Hot Encoding
    encoder = LabelBinarizer()
    encoder.fit(train_labels)
    train_labels = encoder.transform(train_labels)
    test_labels = encoder.transform(test_labels)

    # Changing to float32, so it can be multiplied against the features in TensorFlow, which are float32
...
assert is_features_normal, 'Skipped step to normalize the features'
assert is_labels_encod, 'Skipped step to One-Hot Encode the labels'

# Getting randomized datasets for training and validation
...
# Setting the features and labels tensors
features = tf.placeholder(tf.float32)
labels = tf.placeholder(tf.float32)

# SetTing the weights and bias tensors
...
# Creating an operation that initializes all variables
...
# Change if there are memory restrictions
batch_size = 128
...
with tf.Session() as session:
    ...
    for epoch_i in range(epochs):
        ...
        for batch_i in batches_pbar:
            # Getting a batch of training features and labels
        ...
        # Checking accuracy against Validation data
        validation_accuracy = session.run(accuracy, feed_dict=valid_feed_dict)
...
# Accuracy measured against the test set
test_accuracy = 0.0
...
print('Perfect, The Test Accuracy is {}'.format(test_accuracy))